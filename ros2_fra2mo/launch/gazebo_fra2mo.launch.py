...
def generate_launch_description():
    ...
    world_file = os.path.join(get_package_share_directory('ros2_fra2mo'), "worlds", "airport_runway.sdf")
    ...
    # Bridge principale per cmd_vel, odometry, lidar, clock
    bridge = Node(
    ...
    # Bridge separato per la camera (come nel tuo iiwa!)
    bridge_camera = Node(
        package='ros_gz_bridge',
        executable='parameter_bridge',
        arguments=[
    ...
    nodes_to_start = [robot_state_publisher_node, joint_state_publisher_node, *ign, bridge, 
                      bridge_camera, odom_tf]
    
    # NB: nodo ign_clock_bridge rimosso - causava conflitto clock con bridge principale e provocava blink della mappa su rviz impedendo una corretta localizzazione
